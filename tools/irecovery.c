...
static void print_iboot_info(irecv_client_t client)
{
	...
	if (modecheck == IRECV_E_SUCCESS){
		...
		if (strcmp(mode_to_str(mode),"Recovery") == 0){
			...
			if (devinfo)
			{
				// printf("serial_string: %s", devinfo->serial_string);
				irecv_error_t error = 0;
				...
				else if (strstr(response, "iBoot-8422.120") != NULL || strstr(response, "iBoot-8422.122") != NULL )
				{
					baseIos = (char *)"16.5";
				}
				else if (strstr(response, "iBoot-8422.140") != NULL || strstr(response, "iBoot-8422.142") != NULL )
				{
					baseIos = (char *)"16.6";
				}
				else if (strstr(response, "iBoot-10151.0") != NULL || strstr(response, "iBoot-10151.2") != NULL )
				{
					baseIos = (char *)"17.0";
				}
				else 
				...
				{
					printf("Could not get iboot info?!\n");
				}
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:vVhrsmnc:f:e:k:qab", longopts, NULL)) > 0)
	{
		switch (opt)
		{
		case 'i':
