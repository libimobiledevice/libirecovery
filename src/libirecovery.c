...
static void iokit_cfdictionary_set_int(CFMutableDictionaryRef dict, const void *key, SInt32 value)
{
	CFNumberRef numberRef;
...
#ifdef IOKIT
	UInt8 index;
	(*client->handle)->USBGetSerialNumberStringIndex(client->handle, &index);
...
static irecv_error_t iokit_usb_open_service(irecv_client_t *pclient, io_service_t service) {
	...
	if (serialString) {
		CFStringGetCString(serialString, client->serial, sizeof(client->serial), kCFStringEncodingUTF8);
...
irecv_error_t irecv_open_location(irecv_client_t *pclient, UInt32 locationID) {
	...
	if (locationID == 0) {
		debug("%s: locationID parameter is 0\n", __func__);
		return IRECV_E_INVALID_INPUT;
	}
	...
	*pclient = NULL;
	...
	iokit_cfdictionary_set_int(matchingDict, CFSTR("locationID"), locationID);

	service = IOServiceGetMatchingService(kIOMasterPortDefault, matchingDict);
	if (service == IO_OBJECT_NULL)
		return IRECV_E_UNABLE_TO_CONNECT;

	pidNumber = IORegistryEntryCreateCFProperty(service, CFSTR(kUSBProductID), kCFAllocatorDefault, 0);
	if (pidNumber != NULL) {

		CFNumberGetValue(pidNumber, kCFNumberSInt16Type, &pid);
		switch (pid) {
